...
def run():
    ...
    while(DONE == False):
        ...
        if len(blackcnts) > 0:
            ...
            print("first key hg: ", hg)
            print("first key wg: ", wg)
    ...
    #SECOND HALF!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        #crop = frame[0:height, int(width/2):width] #640
        ...
        erode = cv2.inRange(hsv, lower_black, upper_black)
        kernel = np.ones((2, 2), np.uint8)
        #erode = cv2.morphologyEx(mask, cv2.MORPH_ERODE, kernel)
        kernel = np.ones((3, 3), np.uint8)
        dilatemask = cv2.dilate(erode, kernel, iterations=1)
        ...
        if len(blackcnts) > 0:
            ...
            print("second key hg: ", hg)
            print("second key wg: ", wg)
            if 125 > hg > 100 and 25 > wg > 10:
                b2ready = True
                black2x = xg + offset
                black2y = yg
