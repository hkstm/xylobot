...
class Window(Frame):
    def init_window(self):
        ...
        c = 0.4714
        birds_eye_view.create_rectangle(left + 0 * (keywidth + division), top + 0 * c,
                                        left + 0 * (keywidth + division) + keywidth, bottom - 0 * c, fill="blue")
        birds_eye_view.create_rectangle(left + 1 * (keywidth + division), top + 1 * c,
                                        left + 1 * (keywidth + division) + keywidth, bottom - 1 * c, fill="green")
        birds_eye_view.create_rectangle(left + 2 * (keywidth + division), top + 2 * c,
                                        left + 2 * (keywidth + division) + keywidth, bottom - 2 * c, fill="yellow")
        birds_eye_view.create_rectangle(left + 3 * (keywidth + division), top + 3 * c,
                                        left + 3 * (keywidth + division) + keywidth, bottom - 3 * c, fill="orange")
        birds_eye_view.create_rectangle(left + 4 * (keywidth + division), top + 4 * c,
                                        left + 4 * (keywidth + division) + keywidth, bottom - 4 * c, fill="red")
        birds_eye_view.create_rectangle(left + 5 * (keywidth + division), top + 5 * c,
                                        left + 5 * (keywidth + division) + keywidth, bottom - 5 * c, fill="purple")
        birds_eye_view.create_rectangle(left + 6 * (keywidth + division), top + 6 * c,
                                        left + 6 * (keywidth + division) + keywidth, bottom - 6 * c, fill="white")
        birds_eye_view.create_rectangle(left + 7 * (keywidth + division), top + 7 * c,
                                        left + 7 * (keywidth + division) + keywidth, bottom - 7 * c, fill="darkblue")
        side_view.create_rectangle(width / 2 - multiplier * 5.53, height + 100 - multiplier * xylophone_height,
                                   width / 2 + multiplier * 5.53, height + 100, fill="blue")
        ...
        s_line = side_view.create_line(width / 2 + multiplier * (distance), height + 100,
                                       ...
                                       width / 2 + multiplier * (distance + lower_arm_length * math.cos(
                                           math.radians(lower_joint_angle)) * math.cos(math.radians(direction))),
                                       height + 100 - multiplier * (base_length + lower_arm_length * math.sin(
        ...
        if (type == "forwards"):
            ...
            while (not completed):
                ...
                yLeg = (height + 100 - multiplier * xylophone_height) - (height - multiplier * (
                            base_length + lower_arm_length * math.sin(math.radians(lower_joint_angle))))
                ...
                yLeg = (height + 100 - multiplier * xylophone_height + yDifference) - (
                            height - multiplier * base_length)
